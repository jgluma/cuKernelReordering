/**
 * @file SobolQRNG_kernel.cu
 * @details This file describes the kernel and device functions for a SobolQRNG task.
 * @author Antonio Jose Lazaro Munoz.
 * @date 20/02/2016
 */
#ifndef _SOBOLQRNG_KERNEL_H_
#define _SOBOLQRNG_KERNEL_H_

#include "SobolQRNG_var.h"

__global__ void sobolGPU_kernel(unsigned n_vectors, unsigned n_dimensions, unsigned *d_directions, float *d_output)
{
    __shared__ unsigned int v[n_directions_SOBOLQRNG];

    // Offset into the correct dimension as specified by the
    // block y coordinate
    d_directions = d_directions + n_directions_SOBOLQRNG * blockIdx.y;
    ...
    // first 32 (n_directions_SOBOLQRNG) threads need participate.
    if (threadIdx.x < n_directions_SOBOLQRNG)
    {
    ...
    if (i0 < n_vectors)
    {
        d_output[i0] = (float)X * k_2powneg32_SOBOLQRNG;
    }
    ...
    for (unsigned int i = i0 + stride ; i < n_vectors ; i += stride)
    {
        ...
        d_output[i] = (float)X * k_2powneg32_SOBOLQRNG;
    }
}
